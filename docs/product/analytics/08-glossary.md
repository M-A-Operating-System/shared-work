...
| **Basel III/IV mode** | — | Compliance governance mode enforcing capital adequacy regulatory requirements. Triggers enhanced **compliance provenance record** generation, regulatory snapshot writes, and stress scenario classification. |
...
| **Column mask** | — | Entitlement rule applied by **RAPL** that suppresses or redacts specific field values based on the user's access policy. Applied before results are returned. |
...
| **Data affinity** | — | Logical data domain declared by each metric in the **SMR** (e.g., `portfolio`, `risk_metrics`, `regulatory`). Used by the **FQE** to route each part of a query to the correct registered backend. |
...
| **Dimension hierarchy** | — | Ordered set of dimension levels that support aggregation and drilldown (e.g., geography: country → region → global). Declared in the **SMR**. |
| **Drilldown** | — | Governed decomposition of a computed metric result into a finer dimension level. Produces a new governed result scoped to the selected dimension value. |
...
| **Export gate** | — | Lock applied to compliance-escalated results that prevents export until the complete **compliance provenance record** has been confirmed written and sealed. |
...
| **Field ceiling** | — | Entitlement rule restricting the data classification level of fields visible to a user or agent. Fields above the ceiling are excluded before query execution. |
| **Governance floor** | — | Minimum governance configuration that cannot be lowered by any administrator. Governance floors are architectural properties of the platform. There is no bypass mode. |
| **High-Quality Liquid Assets** | HQLA | **LCR** numerator. Assets that can be readily converted to cash under a stressed scenario. |
...
| **Liquidity Coverage Ratio** | LCR | Basel III regulatory metric: high-quality liquid assets divided by 30-day net stressed cash outflows. A compliance-relevant metric by definition. |
...
| **MiFID II mode** | — | Compliance governance mode for Markets in Financial Instruments Directive II. Enforces PII-adjacent dimension logging, best-execution timeframe requirements, and transaction reporting records. |
