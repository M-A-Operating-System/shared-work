...
| **Semantic Data Context Store (DCS)** | Pre-existing external component — the organisation's authoritative store for semantic definitions. The Analytics Platform extends the DCS to manage analytical metric definitions, making it the single source of truth for the SMR rather than maintaining a separate definition store. |
