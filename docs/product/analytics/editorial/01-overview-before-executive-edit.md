<!-- Working archive, excluded from publication. Preserved in full before the executive rewrite. Reason: implementation examples and repeated architectural explanation obscured the executive argument. Historical draft material, not the current specification. -->
...
The regulatory dimension sharpens this further. An analytical result in a regulated organization is not just a number: it is an assertion. Organizations commonly need to explain how reported figures were calculated, preserve the definition in force at the time, and reproduce the supporting evidence. The exact legal and supervisory obligations vary by jurisdiction, activity, and submission; this architecture is designed to support those obligations but does not, by itself, establish compliance.
...
In practice: a portfolio manager asks "show me portfolio returns versus benchmark for my equity portfolios this quarter" in plain English and receives a governed, role-constrained result with a computation record attached. A data science pipeline extracts millions of rows of position data under the same entitlement and audit controls. A treasury analyst produces an LCR figure for a regulatory submission and receives a signed evidence package designed to support regulatory review. Whether that package satisfies a particular obligation remains a deployment-specific compliance determination.
...
The **Analytics Engine** is the platform's computation core. Given the same resolved request, immutable data snapshot, metric and configuration versions, entitlement snapshot, engine version, timezone, locale, rounding rules, and ordering rules, it produces the same computed values. Probabilistic AI steps do not participate in the calculation once intent has been resolved.
...
|-----------|--------------|
| **P1 — Semantic abstraction** | The normal AI query interface exposes registered semantic concepts rather than physical database schemas. Physical details are limited to explicitly authorized operational and audit interfaces; metadata, errors, timing, and aggregates remain potential disclosure channels that must be controlled. |
...
Most queries require standard governance: approved analytics and metrics definitions, entitlement enforcement, and a provenance record. Some queries require more: when a request involves a metric flagged as compliance-relevant and is made for a compliance-driven purpose, the platform escalates to the enhanced compliance artifact tier. Two independent signals trigger escalation: the metric's compliance-relevant flag and the Intent Resolution Agent's classification of the stated purpose. When both signals are present, the platform produces a signed evidence package designed for compliance review. This classification does not replace required authorization, human approval, or jurisdiction-specific controls.
...
Once execution completes and the result is verified, the platform seals a compliance provenance record and writes it to storage configured with an enforced retention policy. The record covers the governed computation chain while minimizing unnecessary personal data and sensitive result content. A digital signature over a canonical representation makes subsequent alteration of signed fields detectable to a verifier holding the corresponding public key; it does not prove the original data or calculation was correct. The export gate remains locked until the record is durably stored and its signature is verified.
