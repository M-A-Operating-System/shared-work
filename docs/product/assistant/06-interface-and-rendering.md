...
| Expand | Full input parameters, HTTP status, latency, and result detail |
| Write operations | Replaced by a write proposal card before the call is issued |
| Error | Error status and detail shown; model surfaces the failure in plain language |

![Tool call disclosure — rendered collapsed and expanded disclosure card alongside its raw event data](./samples/renderer-previews/10-tool-disclosure.png)
...
![Feedback request card — rendered interactive option buttons alongside its raw JSON source](./samples/renderer-previews/08-feedback-card.png)
...
![Math expression — rendered KaTeX display equations alongside their raw LaTeX source](./samples/renderer-previews/05-math.png)
...
| Tables | Rendered as formatted HTML tables (no sort or filter — use ` ```csv ` for the full data table component) |
| Links | External links open in a new tab; `@`-binding chips render as interactive pills |

![Rich markdown prose — rendered formatted document with headings, bullets, and inline code alongside its raw markdown source](./samples/renderer-previews/06-markdown.png)
