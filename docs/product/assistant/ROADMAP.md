...
| Item | Description |
|------|-------------|
| Session summary injection | At session start, optionally inject a compressed summary of the user's recent conversations (last 30 days) as a labelled `[Recent context]` block. Opt-in per session. Transparent — block visible and dismissible. Generated by Claude Haiku 4.5. |
...
| Extended thinking mode | Expose Claude Opus 4.7's extended thinking (reasoning budget) as an optional session setting. Displayed as a collapsible `<thinking>` block in the thread. Host configures whether this option is available. |
