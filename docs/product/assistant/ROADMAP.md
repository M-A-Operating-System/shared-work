...
This file captures planned enhancements beyond the current release. Items here are named and scoped at a level sufficient for future prioritization. Each requires a Product Design Document and platform team approval before build begins.
...
| Item | Description |
|------|-------------|
| Session summary injection | At session start, optionally inject a compressed summary of the user's recent conversations (last 30 days) as a labeled `[Recent context]` block. Opt-in per session. Transparent — block visible and dismissible. Generated by the fast-tier model. |
| Context globbing | User-initiated: pull selected context from one or more past conversations into a new session. Requires a conversation selection UI, a context budget allocator, and provenance labeling. |

**Pre-requisites:** Auto-summarization infrastructure (already in v1); per-tenant conversation memory RAG (above).

---

## Phase 2 — Extended config capabilities

**Objective:** Give host applications more control over conversation behavior and user experience.
...
| Config-as-code | CI/CD integration that treats the application config as a versioned artifact. Config changes trigger validation and deployment via the SDK rather than the Config Editor UI. |
